...
class PedidosController {
    ...
    public ResponseEntity<Pedidos> getById(@PathVariable("id") long id) {
        Optional<Pedidos> existingItemOptional = repository.findById(id);

        return existingItemOptional.map(pedidos -> new ResponseEntity<>(pedidos, HttpStatus.OK)).orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }

    @PostMapping
    public ResponseEntity<Pedidos> create(@RequestBody Pedidos item) {
        try {
            Pedidos savedItem = repository.save(item);
